...
public class VacuumGame {
  ...
  /**
   * Gets the number of Column in the grid.
   * 
   * @return int
   */ 
  ...
  /** Gets the sprite in the grid.
   * @param row of the sprite
   * @param col of the sprite
   * @return Sprite
   */
  ...
  /** Moves the Correct vacuum to its next location. Also randomly moves the dust.
   * @param nextMove of type char of the key pressed.
   */
  ...
  private void movingTo(Vacuum vc, char nextMove) {
    ...
    // check if the next move is a legal move
  ...
  private void createDust(int row, int col) {
    Dust dirt;
    // check if the dust is in the dusts list.
    if (!checkdust(row, col)) {
      // create new dust and add it's Cell and to it's list of dusts
      dirt = new Dust(row, col, Constants.DUST_SCORE);
      grid.setCell(row, col, dirt);
      dusts.add(dirt);
    } else {
      // get the correct dust and put it back to its corresponding cell
  ...
  private void refreshDust() {
    // loop over every dust, check if it is not showing on the gird 
    // and add it the the grid
  ...
  private Dust getDust(int row, int col) {
    // loop over every  dust in the list 
    // and return the dust corresponding to this location
  ...
  private boolean checkdust(int row, int col) {
    // loop over each dust and check if it exists in this cell.
    boolean res = false;
    for (int d = 0; d < dusts.size(); d++) {
      Dust dust = dusts.get(d);
      if (dust.getRow() == row && dust.getColumn() == col) {
  ...
  /**Checks if gameOver happened by checking if dusts and dustBalls list is empty.
   * @return boolean
   */
